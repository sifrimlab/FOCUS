  ...
  <div class="max-w-4xl mx-auto px-4 py-8">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 mb-6">
      ...
      <div class="p-4">
        ...
        <div v-else class="flex flex-wrap gap-2">
          <button
            ...
            :key="sampleId"
            @click="toggleSample(sampleId)"
            :class="[
              'flex items-center gap-2 px-3 py-2 rounded-lg border-2 transition-all duration-150 select-none focus:outline-none focus:ring-2 focus:ring-offset-1',
              isIncluded(sampleId)
                ? 'border-emerald-500 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/40 dark:hover:bg-emerald-950/60 focus:ring-emerald-400'
                : 'border-red-400 bg-red-50 hover:bg-red-100 dark:bg-red-950/40 dark:hover:bg-red-950/60 focus:ring-red-400'
            ]"
          >
            <span
              :class="['w-2 h-2 rounded-full flex-shrink-0 transition-colors duration-150', isIncluded(sampleId) ? 'bg-emerald-500' : 'bg-red-400']"
            ></span>
            <span
              :class="['font-mono text-sm font-medium transition-colors duration-150', isIncluded(sampleId) ? 'text-emerald-800 dark:text-emerald-200' : 'text-red-700 dark:text-red-300']"
            >{{ sampleId }}</span>
          </button>
        </div>
      </div>
      ...
      <div class="px-5 py-2 border-t border-gray-100 dark:border-gray-700">
        <p class="text-xs text-gray-400 dark:text-gray-500">Click a card to toggle a sample on or off. Disabled samples are fully skipped during processing.</p>
      </div>
    </div>
