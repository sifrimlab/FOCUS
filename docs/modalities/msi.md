...
The `.imzML` / `.ibd` files always live in a `pos/` or `neg/` subdirectory of the modality folder, never directly in it. There is one subdirectory per acquired ion mode.
...
!!! note "Config defaults and direct API calls agree"
    The defaults above are applied both by the pipeline's settings extractor (`_extract_msi_settings`) and by the `MsiDataset.process_dataset()` signature, so there is no divergence between config runs and direct Python calls. Enable `detect_background` (together with a `lipid_annotation_db`) when you want a tissue/background flag; leave it off if all spots are known to be tissue or to skip the GMM/Otsu step for speed.

    `mass_tolerance` must be a Python `int`. Passing a float (e.g. `10.0`) raises `ValueError: mass_tolerance must be a positive integer representing ppm.`
...
`spot_interpolation` performs Gaussian-weighted interpolation: for each anchor spot (from the reference modality), all MSI spots that fall within the anchor's spatial footprint are averaged, weighting each by `exp(−d²/2σ²)` of its distance `d` to the anchor centre, so nearer spots count for more. The result is a new feature vector at each anchor position.

```yaml
registration_type: spot_interpolation
```

`spot_aggregation` uses the same footprint but **sums** the MSI spots inside it instead of averaging them: equal weight, no Gaussian kernel, and no normalization. Use it for subcellular-resolution acquisitions, where one anchor spot covers many MSI spots. See [Registration](../pipeline/registration.md#spot_aggregation).
...
=== "Dual ion mode"
    ...
    The processing settings are identical. There is no ion mode switch in the configuration. Provide a complete `.imzML` + `.ibd` pair in both the `pos/` and `neg/` subdirectory of each sample; FOCUS detects the dual-mode layout from those files.
