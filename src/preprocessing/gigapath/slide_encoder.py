...
class LongNetViT(nn.Module):
    ...
    # Modified forward function to return context enriched patch tokens
    def forward(self, x, coords, all_layer_embed=False, context_enriched_patch_tokens=False):
        """
        ...
        context_enriched_patch_tokens: bool
            Whether to return context enriched patch tokens or the CLS slide token
        """
        # embed patches
        ...
        for x in x_list:
            if self.global_pool:
                ...
                x = self.norm(x)
                if context_enriched_patch_tokens == False:
                    outcome = x[:, 0]
                else:
                    outcome = x[:, 1:, :]
